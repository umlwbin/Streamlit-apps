                ...
                if c==1:
                    ...
                    try:
                        ...
                        if unnamed_cols:
                            ...
                            if unnamed_cols_num==emptyCols:
                                if not func.__name__=='file_cleanup': #for every func except file_cleanup, prompt them to run the file cleanup 
                                    left, right = st.columns([0.8, 0.2])
                                    left.warning(f'This file(s) has {emptyCols} empty columns. They will be removed. Perhaps select the "Tidy File Checker" function first from the above options', icon="⚠️",)
...
def file_cleanup(datafiles,cols):
    ...
    st.markdown('')
    st.markdown('Before beginning file clean-up, are there any Nan representations in your file(s) that are not in the following list?')
    str_Nan_list=['NA', '?', 'N/A', '" "', 'np.nan', 'None', 'Nan', 'NaN']
    nan_str_df=pd.DataFrame(str_Nan_list)
    st.table(nan_str_df.T)
    st.markdown('If yes, enter the Nan representation(s) below. Add multiple if needed by clicking **Add** after each entry.')
    st.markdown('When you are finished, or if you have nothing to enter, click **Continue**')

    nans=[]
    left, right = st.columns([0.2, 0.8])
    user_nan=left.text_input('Enter NaN representation',label_visibility='hidden')

    left2, middle, right2=st.columns([0.2,1, 1])
    #If add button is pressed, add the Nan entered by user
    if left2.button("Add"):
    ...
    if middle.button("Continue",on_click=click_button):
        df_list=[]
        for file in datafiles: # Loop through all the files
        ...
        st.markdown(f'✅ All NaN representations were converted to blank spaces')
