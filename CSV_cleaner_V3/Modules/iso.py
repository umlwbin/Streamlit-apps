...
def convert_dateTime_widgets(cols):
    ...
    date_time_col= col1.selectbox(label='Column',options=list(cols),index=None, key='select7')# name of the date_time column
    col1.button("Next", type="primary", key='convertISONext1_WidgetKey')

    if st.session_state.get("convertISONext1_WidgetKey"): # Next button is clicked
        if date_time_col:
            task_inputs = {"date_time_col": date_time_col}                  
            return task_inputs

        else:
            st.error('Please select date-time column', icon="🚨")
        

def convert_func(df, date_time_col):
    ...
    try: 
        ...
        if not df['temp_date'].empty and not df['temp_time'].empty:
            ...
            df[new_dt_colName]=df[new_dt_colName].dt.strftime('%Y-%m-%dT%H:%M:%S')
