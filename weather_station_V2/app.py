...
if 'radioOptions' not in st.session_state:
    st.session_state.radioOptions= False

if 'oneVarClick' not in st.session_state:
    st.session_state.oneVarClick = False
...
@st.cache_data(show_spinner="Grabbing the latest weather station files and doing some cleaning...This might take a few seconds..")
def grab_data():
...
def options(stl_avgs,eccc_df, newdf):
    ...
    def onClick():
        st.session_state.radioOptions=True
        st.session_state.oneVarClick = False
        st.session_state.compare_click = False
    ...
    index=None,on_change=onClick)

    if st.session_state.radioOptions:
...
def select_var(stl_avgs,newdf):
    ...
    def click_button():
        st.session_state.oneVarClick = True
    ...
    if st.session_state.oneVarClick:
        if var:
            # Call next function
            plot(stl_avgs,var)
        else:
...
def compare(newdf):
    ...
    if st.session_state.compare_click:
        if var:
            # Call next function
